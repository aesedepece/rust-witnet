...
impl JsonRpcClient {
    ...
    pub fn reconnect(&mut self, ctx: &mut <Self as Actor>::Context) {
        ...
        let (_handle, socket) = TcpSocket::new(self.url.as_str())
            ...
            .unwrap();
        log::info!("TCP socket is now connected to {}", self.url);
        self._handle = _handle;
        self.socket = socket;

        // Recover active subscriptions
        let mut active_subscriptions = self.active_subscriptions.lock().unwrap().clone();
        log::debug!(
...
impl Handler<Request> for JsonRpcClient {
    ...
    fn handle(&mut self, request: Request, _ctx: &mut Self::Context) -> Self::Result {
        let Request {
            method,
            params,
            timeout,
